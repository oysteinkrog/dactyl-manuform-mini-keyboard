...
(def tenting-angle (/ π 14))            ; or, change this for more precise tenting control
(def column-style
...
(defn column-offset [column] (cond
                               (= column 2) [0 2.82 -4.5]
                               (>= column 4) [0 -16 5.64]            ; original [0 -5.8 5.64]
                               :else [0 0 0]))
...
(defn thumb-tr-place [shape]
  (->> shape
       ...
       (translate [-23 -8 2]))) ; original 1.5u  (translate [-12 -16 3])
(defn thumb-tl-place [shape]
  (->> shape
       ...
       (translate thumborigin)
       (translate [-37 -15.5 -4]))) ; original 1.5u (translate [-32 -15 -2])))
(defn thumb-bl-place [shape]
  (->> shape
       ...
       (translate thumborigin)
       (translate [-49 -24 -10]))) ;        (translate [-51 -25 -12])))
...
(def case-walls
  (union
   ...
   (wall-brace thumb-bl-place  0 -1 web-post-bl thumb-bl-place  0 -1 thumb-post-br)
   (wall-brace thumb-bl-place  0 -1 web-post-br thumb-tl-place  0 -1 thumb-post-br)
...
(defn screw-insert-all-shapes [bottom-radius top-radius height]
  (union (screw-insert 0 0         bottom-radius top-radius height [12 11 0])
         ...
         (screw-insert 3 lastrow bottom-radius top-radius height [11 5 0])
         (screw-insert lastcol cornerrow bottom-radius top-radius height [-4 -10 0])
...
(spit "things/single-plate.scad"
      (write-scad (translate [0 0 3] single-plate)))
